...
var timer = setInterval(function () {
  ...
  setTimeout(function () {
    for (var i = 0; i < divs.length; i++) {
      divs[i].style.top = 50 + '%';
      divs[i].style.left = 50 + '%';//每个元素都放置在浏览器的中间重叠
      divs[i].style.opacity = 0;//并设置透明隐藏
      divs[i].style.transform = 'translate(-50%,-50%)';//x y轴同时移动
    }
  }, speed)
}, speed*2)
...
function a() {
  return Math.floor(10 * Math.random() + 1)/10;//1-11
}
// 圆角
function br() {
